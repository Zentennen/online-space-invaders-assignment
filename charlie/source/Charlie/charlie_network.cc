...
namespace charlie {
   namespace network {
      ...
      {
          auto first = str.find('.');
          host_ |= (std::stoul(str.substr(0, first))) << 24;
          auto second = str.find('.', first + 1);
          host_ |= (std::stoul(str.substr(first + 1, second - first))) << 16;
          auto third = str.find('.', second + 1);
          host_ |= (std::stoul(str.substr(second + 1, third - second))) << 8;
          auto fourth = str.find('.', third + 1);
          host_ |= (std::stoul(str.substr(third + 1, fourth - third)));
          auto port = str.find(':');
          port_ = static_cast<uint16>(std::stoul(str.substr(port + 1)));
      }
      ...
      bool UDPSocket::get_address(const UDPSocket &socket, IPAddress &address)
      {
         ...
         auto err = getsockname(socket.id_, (sockaddr*)&name, &name_size);
         if (err < 0) {
            return false;
         }
...
//         , buffer_{}
      ...
      namespace {
         template <typename T>
         inline bool can_write(NetworkStreamWriter &writer, const T &value)
         {
            bool b = (writer.at_ - writer.data()) + sizeof(T) <= sizeof(writer.stream_.buffer_);
            writer.error = "Could not write";
            return b;
         }

         inline bool can_write_bytes(NetworkStreamWriter &writer, const uint64 length)
         {
            bool b = (writer.at_ - writer.data()) + length <= sizeof(writer.stream_.buffer_);
            writer.error = "Could not write bytes";
            return b;
         }
      ...
      bool NetworkStreamReader::hasData() const
      {
          return can_read_bytes(*this, 1);
      }
      ...
      void Connection::receive(NetworkStreamReader &reader)
      {
         ...
         if (listener_) {
            //listener_->on_acknowledge(this, acknowledge_, acknowledge_bits_);
             listener_->on_acknowledge(this, packet.acknowledge_, packet.ack_bits_);
         } 
      ...
      void Service::perform_periodic_timeout_check(const Time &time)
      {
         if ((time - last_timeout_check_) >= Time(0.1)) {
                ...
                )
            {
                ...
                if (connection->has_timedout(time)) {
